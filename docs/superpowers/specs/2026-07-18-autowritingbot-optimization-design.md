...
**已撤回的佐证：** 初稿曾把 `dist/` 中并存两个不同 hash 的 JS 当作构建产物混存的证据。经对照实验证伪——向空目录做一次干净构建同样产出两个 JS，那是正常的代码分割（懒加载 chunk），与陈旧残留无关。此条不再作为依据。

**此为假说，未经证实。** 剩余依据只有两条：`restart` 无效而停机再启有效（说明变量不在服务端），以及 `index.html` 确实缺少缓存头。故分两步，验证不可跳过。
